...
// Factory preset captured from the user's saved browser settings, 2026-09-13.
struct Config { int size=560,fps=60,density=2,mode=0,x=80,y=80; float sensitivity=1.5f,pointSize=1.5f,motion=1.3f,glow=2.3f,opacity=0.9f,innerDensity=0.25f,innerOpacity=0.55f,flowStrength=0.45f,waveStrength=1.6f,waveSpeed=0.55f,physicsFreedom=0.5f,rotationSpeed=0.5f,waveRoughness=1.5f,bloomStrength=1.0f,bloomSpread=1.4f,heatStrength=3.0f,outerWaveInfluence=0.2f,innerBounce=1.8f,aaStrength=0.75f,bloomPulse=1.6f,bloomDecay=0.35f;bool locked=true,visible=true,paused=false,demo=false;std::string color="#74BFFF"; };
...
bool updateConfig(const std::map<std::string,std::string>& values,std::string&error){std::lock_guard<std::mutex>lock(configMutex);auto next=config;
    for(auto&[key,value]:values){if(key=="color"){if(!validColor(value)){error="Invalid color";return false;}next.color=value;continue;}
        ...
        if(key=="size"){ok=range(160,2400)&&v==int(v);next.size=int(v);}else if(key=="fps"){ok=v==30||v==60||v==120;next.fps=int(v);}else if(key=="density"){ok=range(1,3)&&v==int(v);next.density=int(v);}else if(key=="mode"){ok=v==0||v==1;next.mode=int(v);}else if(key=="sensitivity"){ok=range(0.1f,5);next.sensitivity=v;}else if(key=="pointSize"){ok=range(0.5f,4);next.pointSize=v;}else if(key=="motion"){ok=range(0,3);next.motion=v;}else if(key=="glow"){ok=range(0,3);next.glow=v;}else if(key=="opacity"){ok=range(0.1f,1);next.opacity=v;}else if(key=="innerDensity"){ok=range(0,1);next.innerDensity=v;}else if(key=="innerOpacity"){ok=range(0,1);next.innerOpacity=v;}else if(key=="flowStrength"){ok=range(0,2);next.flowStrength=v;}else if(key=="waveStrength"){ok=range(0,3);next.waveStrength=v;}else if(key=="rotationSpeed"){ok=range(0.f,2.f);next.rotationSpeed=v;}else if(key=="waveRoughness"){ok=range(0.f,2.f);next.waveRoughness=v;}else if(key=="bloomStrength"){ok=range(0.f,3.f);next.bloomStrength=v;}else if(key=="bloomSpread"){ok=range(0.3f,2.f);next.bloomSpread=v;}else if(key=="bloomPulse"){ok=range(0.f,3.f);next.bloomPulse=v;}else if(key=="bloomDecay"){ok=range(.1f,1.5f);next.bloomDecay=v;}else if(key=="aaStrength"){ok=range(0.f,1.f);next.aaStrength=v;}else if(key=="innerBounce"){ok=range(0.f,2.f);next.innerBounce=v;}else if(key=="outerWaveInfluence"){ok=range(0.f,1.f);next.outerWaveInfluence=v;}else if(key=="heatStrength"){ok=range(0.f,3.f);next.heatStrength=v;}else if(key=="physicsFreedom"){ok=range(0,1);next.physicsFreedom=v;}else if(key=="waveSpeed"){ok=range(.3f,3);next.waveSpeed=v;}else if(key=="locked"){ok=v==0||v==1;next.locked=v!=0;}else if(key=="demo"){ok=v==0||v==1;next.demo=v!=0;}else {error="Unknown setting";return false;}if(!ok){error="Out of range";return false;}
    }config=next;return true;
}
void loadConfig(){std::map<std::string,std::string>v;for(auto key:{"size","fps","density","mode","sensitivity","pointSize","motion","glow","opacity","locked","color","innerDensity","innerOpacity","flowStrength","waveStrength","waveSpeed","physicsFreedom","rotationSpeed","waveRoughness","bloomStrength","bloomSpread","heatStrength","outerWaveInfluence","innerBounce","aaStrength","bloomPulse","bloomDecay"}){wchar_t value[128];GetPrivateProfileStringW(L"visual",widen(key).c_str(),L"",value,128,iniPath.c_str());if(*value){std::string error;updateConfig({{key,narrow(value)}},error);}}
...
void applyWindow(){Config c;{std::lock_guard<std::mutex>lock(configMutex);clampPlacement(config);c=config;}
    LONG_PTR ex=WS_EX_TOOLWINDOW|WS_EX_TOPMOST|WS_EX_NOREDIRECTIONBITMAP|WS_EX_LAYERED;if(c.locked)ex|=WS_EX_TRANSPARENT|WS_EX_NOACTIVATE;SetWindowLongPtrW(windowHandle,GWL_EXSTYLE,ex);SetLayeredWindowAttributes(windowHandle,0,255,LWA_ALPHA);int pad=glowPadding(c);SetWindowPos(windowHandle,HWND_TOPMOST,c.x-pad,c.y-pad,c.size+2*pad,c.size+2*pad,SWP_NOACTIVATE|SWP_FRAMECHANGED);ShowWindow(windowHandle,c.visible?SW_SHOWNOACTIVATE:SW_HIDE);saveConfig();
}
...
int WINAPI wWinMain(HINSTANCE instance,HINSTANCE,PWSTR args,int){if(wcsstr(args,L"--self-test"))return Audio::selfTest()&&Rhythm::selfTest()&&Rhythm::waveSelfTest()&&Rhythm::kickSelfTest()&&bloomBreathSelfTest()?0:1;
    ...
    try {PWSTR path=nullptr;check(SHGetKnownFolderPath(FOLDERID_LocalAppData,0,nullptr,&path),"Settings directory");dataDir=std::wstring(path)+L"\\Luma";CoTaskMemFree(path);CreateDirectoryW(dataDir.c_str(),nullptr);iniPath=dataDir+L"\\settings.ini";loadConfig();
        ...
        windowHandle=CreateWindowExW(WS_EX_TOOLWINDOW|WS_EX_TOPMOST|WS_EX_NOREDIRECTIONBITMAP|WS_EX_LAYERED,L"LumaOverlay",L"Luma · 流光",WS_POPUP,config.x,config.y,config.size,config.size,nullptr,nullptr,instance,nullptr);if(!windowHandle)throw std::runtime_error("Cannot create overlay window");
